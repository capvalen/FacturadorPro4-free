<template>
    <el-dialog :title="titleDialog" :visible="showDialog" @open="create" width="30%" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false" append-to-body>
        ...
        <div class="row mt-3">
            <div class="col-md-12">
                <el-input v-model="form.numero_boletas">
                    <template slot="prepend"># Boletas</template>
                        <el-button slot="append" @click="clickSendGenerateBoletas" >Enviar
                            <el-tooltip class="item" effect="dark">
                                <i class="far fa-poll-h"></i>
                            </el-tooltip>
                        </el-button>
                </el-input>
                <small class="form-control-feedback" v-if="errors.numero_boletas" v-text="errors.numero_boletas[0]"></small>
            </div>
        </div>
        <div class="row mt-3">
            <div class="col-md-12">
...
<script>
    export default {
        ...
        methods: {
            ...
            clickSendGenerateBoletas() {
                this.loading = true
                this.$http.post(`/documentos/generador-boletas/${this.recordId}/${this.form.numero_boletas}`)
                    .then(response => {
                        if (response.data.success) {
                            this.$message.success(`Se genero ${this.form.numero_boletas} boletas con éxito`)
                        } else {
                            this.$message.error('Error al generar las boletas')
                        }
                    })
                    .catch(error => {
                        if (error.response.status === 422) {
